...
// Custom storage for handling Map serialization
const customStorage: StateStorage = {
    ...
    setItem: (name, value) => {
        try {
            ...
            if (parsed.state?.selectedWinners instanceof Map) {
                parsed.state.selectedWinners = Array.from(parsed.state.selectedWinners.entries());
            }
...
export const useGameStore = create<GameStore>()(
    persist(
        ...
        {
            name: 'chipless-poker-game-state',
            storage: createJSONStorage(() => customStorage),
            partialize: (state) => ({
                // Persist essential game state (not transient UI states)
                phase: state.phase,
                ...
                showPhaseNotifications: state.showPhaseNotifications,
                isShowdownResolved: state.isShowdownResolved,
                lastTotalPot: state.lastTotalPot,
                smallBlind: state.smallBlind,
                bigBlind: state.bigBlind,
